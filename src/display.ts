...
function resetLabel(resetsIn?: string): string {
...
function quotaBarWidth(contentWidth: number, labelWidth: number, resetWidth: number): number {
...
function printDetail(frame: Frame, label: string, value: string, layout: ColumnLayout | number): void {
...
function printAccountHeader(
  ...
  const tags: string[] = [];
  if (options.plan) tags.push(options.plan.toUpperCase());
  if (options.recommended) tags.push("NEXT");
  if (options.active) tags.push("ACTIVE");
...
function collectRows(usage: AccountUsage): QuotaRow[] {
  const rows: QuotaRow[] = [];
  if (usage.primary) {
  ...
  for (const limit of usage.additionalLimits ?? []) {
    if (limit.primary) rows.push({ label: limit.name, usedPercent: limit.primary.usedPercent, resetsIn: limit.primary.resetsIn });
    if (limit.secondary) rows.push({ label: `${limit.name} (weekly)`, usedPercent: limit.secondary.usedPercent, resetsIn: limit.secondary.resetsIn });
  }
  return rows;
}

function renderApiKeySpend(frame: Frame, snapshot: ApiKeyUsageSnapshot, layout: ColumnLayout): void {
...
function collectCodexLayout(usages: AccountUsage[]): { labels: string[]; resets: (string | undefined)[] } {
...
function displayAccount(frame: Frame, usage: AccountUsage, layout: ColumnLayout, index?: number): void {
  ...
  if (usage.gtoReason) printDetail(frame, "Next pick", usage.gtoReason, layout);
  for (const row of collectRows(usage)) printQuota(frame, row, layout);
  if (usage.credits) {
    printDetail(frame, "Credits", usage.credits.unlimited ? "Unlimited" : `$${usage.credits.balance ?? "0"}`, layout);
  }
  if (usage.apiKeySpend) renderApiKeySpend(frame, usage.apiKeySpend, layout);
...
function collectClaudeRows(profile: ClaudeProfileInfo): QuotaRow[] {
  const rows: QuotaRow[] = [];
  const usage = profile.usage;
  if (!usage) return rows;
  if (usage.five_hour?.utilization != null) rows.push({ label: "5h limit", usedPercent: usage.five_hour.utilization, resetsIn: usage.five_hour.resets_at ? formatResetTime(usage.five_hour.resets_at) : undefined });
  if (usage.seven_day?.utilization != null) rows.push({ label: "7d limit", usedPercent: usage.seven_day.utilization, resetsIn: usage.seven_day.resets_at ? formatResetTime(usage.seven_day.resets_at) : undefined });
  if (usage.seven_day_opus?.utilization != null) rows.push({ label: "Opus (weekly)", usedPercent: usage.seven_day_opus.utilization, resetsIn: usage.seven_day_opus.resets_at ? formatResetTime(usage.seven_day_opus.resets_at) : undefined });
  if (usage.seven_day_sonnet?.utilization != null) rows.push({ label: "Sonnet (weekly)", usedPercent: usage.seven_day_sonnet.utilization, resetsIn: usage.seven_day_sonnet.resets_at ? formatResetTime(usage.seven_day_sonnet.resets_at) : undefined });
  if (usage.extra_usage?.is_enabled && usage.extra_usage.utilization != null) rows.push({ label: "Extra usage", usedPercent: usage.extra_usage.utilization });
...
function collectClaudeLayout(profiles: ClaudeProfileInfo[]): { labels: string[]; resets: (string | undefined)[] } {
...
function displayClaudeProfile(frame: Frame, profile: ClaudeProfileInfo, layout: ColumnLayout, index?: number): void {
  ...
  if (identity && identity !== profile.name) printDetail(frame, "Identity", identity, layout);
  const rows = collectClaudeRows(profile);
  for (const row of rows) printQuota(frame, row, layout);
...
function grokPeriodLabel(profile: GrokProfileInfo): string {
...
function grokQuotaRow(profile: GrokProfileInfo): QuotaRow | undefined {
...
function collectGrokLayout(profiles: GrokProfileInfo[]): { labels: string[]; resets: (string | undefined)[] } {
...
function displayGrokProfile(frame: Frame, profile: GrokProfileInfo, layout: ColumnLayout, index?: number): void {
  ...
  if (quota) printQuota(frame, quota, layout);
  const prepaid = profile.usage?.config?.prepaidBalance?.val;
  if (prepaid !== undefined) printDetail(frame, "Prepaid credits", fmtUsd(prepaid / 100), layout);
  ...
  if (!quota && prepaid === undefined && onDemandUsed === undefined && !identity) printNote(frame, "Usage data unavailable");
}
...
export function renderCombinedUsage(data: CombinedUsage, options: RenderOptions = {}): string[] {
  ...
  const layout = measureLayout(
    [...codex.labels, ...claude.labels, ...grok.labels],
    [...codex.resets, ...claude.resets, ...grok.resets],
    width,
  );
