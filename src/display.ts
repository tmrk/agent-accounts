import type { AccountUsage, ApiKeyUsageSnapshot, ClaudeProfileInfo, DailyUsage, GrokProfileInfo } from "./types.js";
...
function displayGrokProfile(profile: GrokProfileInfo, labelWidth: number, index?: number): void {
  ...
  if (usedPercent !== null) {
    ...
    console.log(
      `  ${DIM}${label}:${RESET} ${renderBar(clamped)} ${usageColor(clamped)}${Math.max(0, 100 - usedPercent).toFixed(1)}% left${RESET}${resets}`,
    );
  }
...
function grokLabelWidth(profiles: GrokProfileInfo[]): number {
  ...
  for (const profile of profiles) {
    ...
    if (profile.usage?.config?.prepaidBalance) width = Math.max(width, "Prepaid credits".length);
...
export function displayGrokProfiles(profiles: GrokProfileInfo[]): void {
  ...
  console.log();
  for (const profile of profiles) displayGrokProfile(profile, width);
}
...
export function displayGrokProfilesNumbered(profiles: GrokProfileInfo[]): void {
  ...
  const width = grokLabelWidth(profiles);
  console.log();
  for (let i = 0; i < profiles.length; i++) displayGrokProfile(profiles[i]!, width, i + 1);
}
