 ...
 */
const MODEL_PRICES: Array<{ prefix: string; inUsdPerM: number; outUsdPerM: number; cachedDiscount?: number }> = [
...
function priceFor(model: string): { inUsdPerM: number; outUsdPerM: number; cachedDiscount: number } | undefined {
...
export function estimateCostUsd(model: string, inputTokens: number, cachedInputTokens: number, outputTokens: number): number | undefined {
...
async function fetchJsonOnce<T>(adminKey: string, path: string, params: Record<string, string | string[]>): Promise<T> {
 ...
 */
async function fetchJson<T>(adminKey: string, path: string, params: Record<string, string | string[]> = {}): Promise<T> {
  const maxAttempts = 4;
  let lastErr: unknown;
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      return await fetchJsonOnce<T>(adminKey, path, params);
    } catch (err) {
      lastErr = err;
...
export async function validateAdminKey(adminKey: string): Promise<{ ok: true; orgId?: string } | { ok: false; error: string }> {
  try {
...
export async function listProjects(adminKey: string): Promise<OpenAIProject[]> {
  const out: OpenAIProject[] = [];
  let after: string | undefined;
  for (let page = 0; page < 20; page++) {
    const params: Record<string, string> = { limit: "100" };
    if (after) params.after = after;
    const res = await fetchJson<PageEnvelope<OpenAIProject> & { last_id?: string }>(
      adminKey,
      "/v1/organization/projects",
      params,
    );
    for (const p of res.data ?? []) out.push({ id: p.id, name: p.name, status: p.status });
...
export async function fetchUsageRollup(
  ...
  days: number,
  projectId?: string,
): Promise<{ daily: DailyUsage[]; topModel?: { model: string; tokens: number } }> {
  const startTime = Math.floor(Date.now() / 1000) - days * 86_400;
  const limit = String(Math.min(days, 30));

  const costsParams: Record<string, string | string[]> = {
    start_time: String(startTime),
    bucket_width: "1d",
    limit,
  };
  const usageParams: Record<string, string | string[]> = {
    start_time: String(startTime),
    bucket_width: "1d",
    limit,
    "group_by[]": ["model"],
  ...
  const [costsRes, usageRes, todayResult] = await Promise.all([
    fetchJson<{ data: CostsResponseBucket[] }>(adminKey, "/v1/organization/costs", costsParams),
    fetchJson<{ data: UsageCompletionsBucket[] }>(adminKey, "/v1/organization/usage/completions", usageParams),
    fetchTodayHourly(adminKey, projectId).catch(() => null),
  ]);

  const byDate = new Map<string, DailyUsage>();
  const ensure = (epochSeconds: number, iso?: string): DailyUsage => {
  ...
  for (const bucket of costsRes.data ?? []) {
    ...
    for (const r of bucket.results ?? []) {
      if (r.amount?.value != null) row.costUsd += Number(r.amount.value) || 0;
    }
  }
  ...
  let topModel: { model: string; tokens: number } | undefined;
...
export async function fetchDailyUsage(adminKey: string, days: number, projectId?: string): Promise<DailyUsage[]> {
 ...
 */
export async function fetchTodayHourly(adminKey: string, projectId?: string): Promise<TodayHourlyResult | null> {
  const now = new Date();
  ...
  const res = await fetchJson<{ data: UsageCompletionsBucket[] }>(
    adminKey,
