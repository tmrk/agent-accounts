...
async function fetchInfos(): Promise<GrokProfileInfo[]> {
...
async function cmdSwitch(name?: string): Promise<void> {
  ...
  if (!selected) {
    ...
    if (matches.length > 1) throw new Error(`Multiple Grok Build profiles match "${name}": ${matches.map(p => p.name).join(", ")}`);
    selected = matches[0]!;
  }
