...
import {
  cleanupGrokInstance,
  createGrokProfile,
  createTempGrokInstance,
  detectGrokCli,
  fetchGrokUsageForPath,
  findGrokProfile,
  getActiveGrokProfile,
  getGrokInstancePath,
  listGrokProfiles,
  readGrokAuth,
  registerGrokProfile,
  removeGrokProfile,
  setActiveGrokProfile,
  validateGrokProfileName,
} from "./grok-store.js";
...
export function parseGrokAddArgs(args: string[]): AddArgs {
  const parsed: AddArgs = { deviceAuth: false };
...
function runInherited(command: string, args: string[], env: NodeJS.ProcessEnv): Promise<number> {
...
export async function loadGrokProfiles(): Promise<GrokProfileInfo[]> {
...
async function cmdAdd(args: string[]): Promise<void> {
  ...
  try {
    ...
    if (name) removeGrokProfile(name); else cleanupGrokInstance(dirName!);
    throw new Error(`Failed to start Grok Build: ${(error as Error).message}`);
  }
  ...
  if (!name) {
    if (findGrokProfile(profileName)) removeGrokProfile(profileName);
...
export async function grokStatus(): Promise<void> {
  displayGrokProfiles(await loadGrokProfiles());
}

function resolveGrokProfileName(name: string): string {
  ...
  if (matches.length === 0) throw new Error(`No Grok Build profile matching "${name}".`);
  if (matches.length > 1) {
...
export function activateGrokProfile(name: string): SwitchOutcome {
  const selected = resolveGrokProfileName(name);
  if (getActiveGrokProfile() === selected) return alreadyActiveOutcome(selected);
...
function cmdRemove(name: string): void {
  if (!name) throw new Error("Usage: aacc grok remove <name>");
  if (!removeGrokProfile(name)) throw new Error(`Grok Build profile "${name}" not found.`);
...
function cmdEnv(): void {
  const active = getActiveGrokProfile();
  if (active) console.log(`export GROK_HOME=${shellQuote(getGrokInstancePath(active))}`);
}

async function cmdRun(name: string | undefined, extraArgs: string[]): Promise<void> {
  const profileName = name || getActiveGrokProfile();
  if (!profileName) throw new Error("No Grok Build profile specified and no active profile set.");
  if (!findGrokProfile(profileName)) throw new Error(`Grok Build profile "${profileName}" not found.`);
  const grokPath = detectGrokCli();
  if (!grokPath) throw new Error("Grok Build CLI not found. Install it from https://x.ai/build.");
  setActiveGrokProfile(profileName);
...
export async function grokMain(args: string[]): Promise<void> {
  const command = args[0];
  if (!command) return cmdPromptSwitch();
  switch (command) {
    case "add":
    case "login":
    ...
    case "-h":
      ...
      return;
    default:
      if (findGrokProfile(command)) return cmdRun(command, args.slice(1));
