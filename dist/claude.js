...
export async function loadClaudeProfiles() {
    ...
    const withCreds = await Promise.all(profiles.map(async (p) => {
        try {
            ...
            return { profile: p, auth: null, credential: null, error: err.message };
        }
    }));
...
async function cmdPromptSwitch() {
    ...
    if (idx >= 0 && idx < infos.length)
        return cmdSwitch(infos[idx].name);
