...
function resetAfterSeconds(window) {
...
export async function fetchUsageWithRetry(auth) {
...
export async function loadAccountUsage(email, auth, isActive) {
    try {
...
export async function refreshAndFetchUsage(auth) {
    return fetchUsageWithRetry(auth);
}
/** Convert raw usage response to display format */
export function formatUsage(email, isActive, usage) {
    const result = {
        email,
        isActive,
    ...
    const extraLimits = (usage.additional_rate_limits ?? []).filter(arl => shouldShowAdditionalLimit(arl, usage.plan_type));
...
export function isReserveAdditionalLimit(limit) {
    const haystack = `${limit.limit_name ?? ""} ${limit.metered_feature ?? ""}`.toLowerCase();
    return haystack.includes("gpt-reserve");
}
export function shouldShowAdditionalLimit(limit, planType) {
    if (isReserveAdditionalLimit(limit))
        return false;
    if (!isSparkAdditionalLimit(limit))
        return true;
    return planHasSparkAccess(planType);
}
function formatLimitWindow(window) {
