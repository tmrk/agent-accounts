...
export function scoreUsageForGto(usage) {
...
export function rankUsagesForGto(usages) {
...
export function compareUsageForGto(left, right) {
    const a = scoreUsageForGto(left);
    const b = scoreUsageForGto(right);
    if (a.hasError !== b.hasError)
        return a.hasError ? 1 : -1;
    if (a.isApiKey !== b.isApiKey)
        return a.isApiKey ? 1 : -1;
    if (a.usableForNewSession !== b.usableForNewSession)
        return a.usableForNewSession ? -1 : 1;
    ...
    if (a.shortResetAfterSeconds !== b.shortResetAfterSeconds) {
        if (a.shortResetAfterSeconds === 0)
            return 1;
        if (b.shortResetAfterSeconds === 0)
            return -1;
